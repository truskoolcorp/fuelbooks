...
function parseInvoicePDF(text, customers, suppliers) {
  ...
  // Strategy 5: reversed "200.000000 200.00 1.0000" from pdf.js
  if (!r.gallons) {
    const revM = text.match(/(\d+\.\d{4,8})\s+([\d,]+\.\d{2})\s+([\d,]+\.\d{4})/);
    if (revM) {
      const a = parseFloat(revM[1]), b = parseFloat(revM[2].replace(/,/g,'')), c = parseFloat(revM[3].replace(/,/g,''));
      if (c < a) { r.gallons = c; r.unitPrice = a; r.lineTotal = b; }
      else { r.unitPrice = a; r.lineTotal = b; r.gallons = c; }
    }
  }
...
export default function EmailIngestModule({ customers, suppliers, invoices, onCreateInvoice, onCreateBill, onCreateExpense, onToast }) {
  ...
  const initEdit = (result) => {
    const e = {
      ...
      entityName: result.direction === "AR"
        ? (result.matchedEntity?.name || result.shipToName || result.billToName || "")
        : (result.matchedEntity?.name || result.vendorName || result.shipToName || ""),
